...
public class Chicken implements IAnimal {
    ...
    public Chicken(String gender, int x, int y) {
        this.gender = gender;
        this.name = "Chicken" + Chicken.count;
        this.x = x;
        this.y = y;
        Chicken.count++;
    }
    ...
    public void makeInteraction(List<IAnimal> animal) {
        for (int j = 0; j < animal.size(); j++) {
            ...
            if ((otherAnimal.getSpecies().equals("Rooster") &&
                ...
                if (distance <= 3) {
                    System.out.println(
                            this.name + " isimli hayvan " + otherAnimal.getName() + " isimli hayvan ile çiftleşti.");
                    if (randomNumber() == 1) {
                        App.zoo.addNewBorn("Chicken", "female", this.x, this.y);
                        ex.add(otherAnimal.getName());
                        break;
                    } else {
                        App.zoo.addNewBorn("Rooster", "male", this.x, this.y);
                        ex.add(otherAnimal.getName());
                        break;
                    }
